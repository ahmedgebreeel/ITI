...
const Cart = () => {
  ...
  const Decrement = (product)=>{
    ...
    const Reset = ()=>{
      ...
      let newProducts = [...products];
      //edit
      newProducts = newProducts.map(product => {return {...product, counter: 0}});
      //setState
      setProducts(newProducts);
  ...
  return (
    <div>
      {products.map(product => <CartItem key={product.name} product = {product} Increment={Increment} Decrement={Decrement}/>)}
      <button onClick={Reset} className="border border-blue-500 m-5 p-3">Reset</button>
    </div>
